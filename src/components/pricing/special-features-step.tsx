...
export default function SpecialFeaturesStep({ features, stepFiveComplete, onFeaturesChange, onBack, onNext }: SpecialFeaturesStepProps) {
  const items = [
    { id: "garage", title: "Garage / Carport", range: "රු.500k - 1.0M" },
    { id: "swimmingPool", title: "Swimming pool", range: "රු.2.5M - 5.0M" },
    { id: "boundaryWall", title: "Boundary wall & gate", range: "රු.700k - 1.5M" },
    ...
    { id: "servantQuarters", title: "Servant quarters", range: "රු.350k - 700k" },
    { id: "waterTank", title: "Underground water tank", range: "රු.200k - 450k" },
  ] as const;

  function toggle(id: keyof SpecialFeaturesState) {
    onFeaturesChange({ ...features, [id]: !features[id] } as SpecialFeaturesState);
  }

  return (
    <div className="space-y-8">
      <div className="space-y-3">
        <h1 className="text-3xl font-semibold tracking-[-0.04em] text-slate-50 sm:text-[2.2rem]">Additional structures & services</h1>
        <p className="max-w-3xl text-sm leading-7 text-slate-400 sm:text-base">Main building එකට අමතරව include කිරීමට අවශ්‍ය features සහ structures තෝරන්න.</p>
      </div>
      ...
      <div className="border-t border-white/10 pt-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button type="button" disabled={!stepFiveComplete} onClick={onNext} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/3 px-6 text-[0.95rem] font-medium text-slate-300/80">Generate estimate</button>
        </div>
      </div>
    </div>
  );
