...
function StarIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path
        d="M12 17.3 6.1 20l1.1-6.3L2 9.6l6.4-.9L12 3l3.6 5.7 6.4.9-5.2 3.9L17.9 20z"
        stroke="currentColor"
        strokeWidth="1"
        strokeLinecap="round"
...
export default function FinishingGradeStep({
  finishing,
  stepFourComplete,
  onFinishingChange,
  onBack,
  ...
  const grades = [
    {
      id: "Budget",
      title: "Budget",
      subtitle: "Cement floors, basic paint, local fittings",
      range: "රු5,500–7,500/sqft",
    },
    {
      id: "Standard",
      title: "Standard",
      subtitle: "Tiles, aluminium windows, painted walls",
      range: "රු8,000–12,000/sqft",
    },
    {
      id: "Luxury",
      title: "Luxury",
      subtitle: "Imported tiles, premium fittings",
      range: "රු13,000–20,000+/sqft",
    },
  ];
  ...
  const interiorOptions = {
    flooring: [
      "Cement / Terrazzo",
      "Ceramic tiles (+රු180k)",
      "Porcelain / Granite (+රු380k)",
      "Marble (+රු700k)",
    ],
    windows: ["Timber frames", "Aluminium (+රු220k)", "UPVC (+රු220k)"],
    electrical: [
      "Standard",
      "Modular switches (+රු150k)",
      "Smart home (+රු500k)",
    ],
    ...
    kitchen: [
      "Basic kitchen",
      "Modern kitchen (+රු200k)",
      "Island / Designer (+රු500k)",
    ],
  };
    ...
    <div className="space-y-8">
      <div className="space-y-3">
        <h1 className="text-3xl font-semibold tracking-[-0.04em] text-slate-50 sm:text-[2.2rem]">
          Finishing & fittings grade
        </h1>
        <p className="max-w-3xl text-sm leading-7 text-slate-400 sm:text-base">
          Overall finishing quality level සහ key interior selections තෝරන්න.
          මේවට total cost range එකට බලපායි.
        </p>
      </div>
        ...
        <div className="mt-6">
          ...
          <div className="grid gap-3 sm:grid-cols-3 mt-3">
            <label className="space-y-2">
              ...
              <select
                value={finishing.flooring}
                onChange={(e) =>
                  onFinishingChange({ ...finishing, flooring: e.target.value })
                }
                className="h-12 w-full rounded-[10px] border border-white/10 bg-white/4 px-4 text-[1rem] text-slate-100"
              >
                {interiorOptions.flooring.map((o) => (
            ...
            <label className="space-y-2">
              ...
              <select
                value={finishing.windows}
                onChange={(e) =>
                  onFinishingChange({ ...finishing, windows: e.target.value })
                }
                className="h-12 w-full rounded-[10px] border border-white/10 bg-white/4 px-4 text-[1rem] text-slate-100"
              >
                {interiorOptions.windows.map((o) => (
                  <option key={o} value={o} className="bg-[#161616]">
                    {o}
                  </option>
                ))}
            ...
            <label className="space-y-2">
              ...
              <select
                ...
                className="h-12 w-full rounded-[10px] border border-white/10 bg-white/4 px-4 text-[1rem] text-slate-100"
              >
                {interiorOptions.electrical.map((o) => (
                  <option key={o} value={o} className="bg-[#161616]">
                    {o}
                  </option>
                ))}
            ...
            <label className="space-y-2">
              ...
              <select
                value={finishing.plumbing}
                onChange={(e) =>
                  onFinishingChange({ ...finishing, plumbing: e.target.value })
                }
                className="h-12 w-full rounded-[10px] border border-white/10 bg-white/4 px-4 text-[1rem] text-slate-100"
              >
                {interiorOptions.plumbing.map((o) => (
                  <option key={o} value={o} className="bg-[#161616]">
                    {o}
                  </option>
                ))}
            ...
            <label className="space-y-2">
              ...
              <select
                value={finishing.ceiling}
                onChange={(e) =>
                  onFinishingChange({ ...finishing, ceiling: e.target.value })
                }
                className="h-12 w-full rounded-[10px] border border-white/10 bg-white/4 px-4 text-[1rem] text-slate-100"
              >
                {interiorOptions.ceiling.map((o) => (
                  <option key={o} value={o} className="bg-[#161616]">
                    {o}
                  </option>
                ))}
            ...
            <label className="space-y-2">
              ...
              <select
                value={finishing.kitchen}
                onChange={(e) =>
                  onFinishingChange({ ...finishing, kitchen: e.target.value })
                }
                className="h-12 w-full rounded-[10px] border border-white/10 bg-white/4 px-4 text-[1rem] text-slate-100"
              >
                {interiorOptions.kitchen.map((o) => (
                  <option key={o} value={o} className="bg-[#161616]">
                    {o}
                  </option>
                ))}
